...
public partial class TreeView : UserControl
{
    ...
    public void ReleaseCapture() => this.ReleaseMouseCapture();
            ...
            case MouseSide.InSide:
            {
                ...
                if (_dragSourceSet.Count > 0 && _dragTargetItem != null)
                    foreach (var source in _dragSourceSet)
                        DoDrag(source, _dragTargetItem);
                // 置空拖动目标
                _dragTargetItem = null;
                // 隐藏目标框
                TargetBorder.IsVisible = false;
                // 更新视图
                Update();
            }
                break;
    ...
    {
        if (_mouseMove.X < 0 || _mouseMove.Y < 0 || _mouseMove.X > Width || _mouseMove.Y > Height)
    ...
    private void UpdateScrollBar()
    {
        ...
        MainScrollBar.ViewportSize = ((int)MainGrid.Height - 20) / _controlHeight;
        // 数据量 > 可视数据量
    ...
    private TreeItem? FindSubItem(TreeItem? parent, string nodeName)
    {
        if (parent == null)
        {
            foreach (var item in TreeRoot.ItemList)
                if (item.Content.ToString() == nodeName)
                    return item;
        }
        else
        {
    ...
    private bool CanDrag()
    {
        if (_dragSourceSet.Count > 0 && _dragTargetItem != null)
        {
            // 有一个能拖动就返回真
            foreach (var source in _dragSourceSet)
                if (CanDrag(source, _dragTargetItem))
                    return true;
        }
